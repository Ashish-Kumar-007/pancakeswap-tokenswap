...
const connectToMetaMask = async () => {
  try {
    // Request MetaMask to connect and request accounts
    ...
    if (accounts.length > 0) {
      ...
      Status.textContent = "Connection to MetaMask failed";
    }
  } catch (error) {
    console.error("Error connecting to MetaMask:", error);
    Status.textContent = "Error connecting to MetaMask";
  }
};
...
const interactWithContract = async () => {
  try {
    ...
    await window.ethereum.request({ method: "eth_requestAccounts" }); // Request user permission to access accounts
    ...
    try {
      ...
      const tx = await pancakeswap.methods
        ...
        .send({ from: accounts[0], gas: 500000 }); // Adjust the gas limit as needed
...
getDataButton.addEventListener("click", interactWithContract);
